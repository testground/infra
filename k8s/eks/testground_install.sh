...
deploy_multus_ds >> $real_path/$start-log/cluster.log
echo "========================"
...
if [[ "$CNI_COMBINATION" == "aws_vpc_cni_weave" ]]
  ...
  echo "Invalid selecton in .env"  >> $real_path/$start-log/erorr.log
  echo "CNI_COMBINATION can't be $CNI_COMBINATION" >> $real_path/$start-log/erorr.log
...
deploy_weave_cni >> $real_path/$start-log/cluster.log
create_weave_networkattachmentdefinition >> $real_path/$start-log/cluster.log
echo "========================"

echo "Now setting role binding..."
deploy_cluster_role_binding >> $real_path/$start-log/cluster.log
echo "========================"
echo ""
echo "Dropping weave iptables rule..."
weave_ip_tables_drop >> $real_path/$start-log/cluster.log
echo "========================"
echo ""
echo "Making multus softlink..."
multus_softlink >> $real_path/$start-log/cluster.log
echo "========================"
echo ""
echo "Creating cluster config file with name $CLUSTER_NAME.yaml"
make_cluster_config >> $real_path/$start-log/cluster.log
echo "========================"
echo ""
echo "Creating nodegroup now, this can also take some time..."
create_node_group >> $real_path/$start-log/cluster.log
echo "========================"
echo ""
...
aws_create_efs_mount_point >> $real_path/$start-log/cluster.log
echo "Your EFS mountpoint DNS is: $efs_dns"
echo ""
echo "Now creating EFS configmap"
create_cm_efs >> $real_path/$start-log/cluster.log
echo ""
echo "Now creating EFS manifest"
create_efs_manifest >> $real_path/$start-log/cluster.log
...
aws_create_ebs >> $real_path/$start-log/cluster.log
echo "EBS created with this volume ID: $ebs_volume "
echo ""
echo "Now making persistent volume on EKS"
make_persistent_volume >> $real_path/$start-log/cluster.log
echo "Done."
echo "========================"

echo "Setting up Redis..."
helm_redis_add_repo >> $real_path/$start-log/cluster.log
echo "Helm Redis repo added"
echo "Now proceeding with Helm install"
helm_infra_install_redis >> $real_path/$start-log/cluster.log
...
tg_daemon_config_map >> $real_path/$start-log/cluster.log
echo "Creating service account..."
tg_daemon_service_account >> $real_path/$start-log/cluster.log
echo "Creating role binding..."
tg_daemon_role_binding >> $real_path/$start-log/cluster.log
echo "Creating the testground service.."
tg_daemon_services >> $real_path/$start-log/cluster.log
echo "Creating svc sync service..."
tg_daemon_svc_sync_service >> $real_path/$start-log/cluster.log
echo "Creating sync service deployment..."
tg_daemon_sync_service >> $real_path/$start-log/cluster.log
echo "Creating sidecar..."
tg_daemon_sidecar >> $real_path/$start-log/cluster.log
echo "Creating testground daemon deployment..."
tg_daemon_deployment >> $real_path/$start-log/cluster.log
